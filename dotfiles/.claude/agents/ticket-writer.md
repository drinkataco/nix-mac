...
### 2. Background / Context  *(optional, often skipped)*
...
### 4. Out of Scope / Non-goals  *(optional, often skipped)*
...
- **Feature/product-focused tickets:** write in **BDD** one or more scenarios to capture the exact requirements. For example:
  ```
  SCENARIO: <name>
    GIVEN <precondition>
    WHEN <action>
...
2. Gather context when it sharpens the ticket — grep the relevant repo(s) for the components, services, or files involved so the Task Breakdown names real things. Look at existing tickets, if provided, to give added context if 
